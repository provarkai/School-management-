...
import { askParentAssistant, type ChatTurn } from "./actions";

const SUGGESTIONS = [
  "How is my child doing this term?",
  "What's my child's fee balance?",
  "How has attendance been recently?",
  "What were the latest scores?",
];
...
export function ParentChat() {
  ...
  function send(message: string) {
    ...
    startTransition(async () => {
      const result = await askParentAssistant(history, trimmed);
        ...
        {history.length === 0 && (
          <div className="space-y-3">
            <p className="text-sm text-zinc-400">
              Ask about your child&rsquo;s attendance, fees, or results. Try one of these:
            </p>
          ...
          <div key={i} className={`flex ${turn.role === "user" ? "justify-end" : "justify-start"}`}>
            <div
              className={`max-w-[85%] whitespace-pre-wrap rounded-lg px-3 py-2 text-sm ${
                turn.role === "user" ? "bg-zinc-900 text-white" : "bg-zinc-100 text-zinc-800"
              }`}
            >
              {turn.content}
            </div>
