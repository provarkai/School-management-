...
import { submitStaffAttendance } from "./actions";
...
export function StaffAttendanceGrid({
  ...
  function setAll(status: AttendanceStatus) {
    setMarks(Object.fromEntries(staff.map((s) => [s.id, status])));
  }
  ...
  function handleSubmit() {
    ...
    const entries = staff
      .filter((s) => marks[s.id])
      .map((s) => ({ staffId: s.id, status: marks[s.id] }));

    startTransition(async () => {
      const result = await submitStaffAttendance(date, entries);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="divide-y divide-zinc-100 overflow-hidden rounded-lg border border-zinc-200 bg-white shadow-sm">
        ...
        {staff.length === 0 && (
          <p className="px-4 py-6 text-center text-sm text-zinc-400">No staff on record.</p>
        )}
      </div>
      ...
      >
        {pending ? "Saving…" : `Submit attendance (${markedCount}/${staff.length})`}
      </button>
    </div>
  );
}
