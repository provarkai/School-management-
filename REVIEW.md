...
live project already has flipped on). The Management API accepts but
silently ignores that field on the free plan, so it stays a one-click
dashboard flip; the app code is already compatible, so flipping it needs no
further changes.
